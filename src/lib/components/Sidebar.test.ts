...
import { sidebarCollapse } from "$lib/stores.svelte";
...
describe("Sidebar", () => {
	...
	it("does not render any doc-type or category pill rows", async () => {
		...
		expect(container.querySelector(".type-filters")).toBeNull();
		expect(container.querySelector(".type-filter-chip")).toBeNull();
		expect(container.querySelector(".category-filters")).toBeNull();
		expect(container.querySelector(".category-filter-chip")).toBeNull();
	});
});
