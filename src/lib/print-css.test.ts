...
describe('print CSS: breadcrumbs hidden', () => {
	it('hides breadcrumb navigation', () => {
		...
		const breadcrumbSection = printBlock.slice(
			printBlock.indexOf('nav[aria-label="Breadcrumb"]')
		);
...
describe('print CSS: compact typography', () => {
	...
	it('.markdown-content font-size is 12pt', () => {
		expect(printBlock).toMatch(
			/\.markdown-content\s*\{[^}]*font-size:\s*12pt\s*!important/
		);
	});

	it('h1 is 18pt', () => {
		expect(printBlock).toMatch(
			/\.markdown-content\s+h1\s*\{[^}]*font-size:\s*18pt\s*!important/
		);
	});

	it('h2 is 15pt', () => {
		expect(printBlock).toMatch(
			/\.markdown-content\s+h2\s*\{[^}]*font-size:\s*15pt\s*!important/
		);
	});

	it('h3 is 13pt', () => {
		expect(printBlock).toMatch(
			/\.markdown-content\s+h3\s*\{[^}]*font-size:\s*13pt\s*!important/
		);
	});

	it('code is 10pt', () => {
		expect(printBlock).toMatch(
			/\.markdown-content\s+code\s*\{[^}]*font-size:\s*10pt\s*!important/
		);
	});

	it('table is 11pt', () => {
		expect(printBlock).toMatch(
			/\.markdown-content\s+table\s*\{[^}]*font-size:\s*11pt\s*!important/
		);
	});
});
...
describe('print CSS: page break control', () => {
	...
	it('prevents break inside blockquotes', () => {
		expect(printBlock).toMatch(
			/\.markdown-content\s+blockquote[\s\S]*?break-inside:\s*avoid/
		);
	});
	...
	for (const rule of criticalRules) {
		it(`${rule.selector} { ${rule.property}: ${rule.value} } has !important`, () => {
			...
			const propRegex = new RegExp(
				`${rule.property}:\\s*${rule.value}\\s*!important`
			);
...
describe('print CSS: metadata 3-row layout', () => {
	it('.doc-meta-row uses flex-direction: column !important', () => {
		expect(printBlock).toMatch(
			/\.doc-meta-row\s*\{[^}]*flex-direction:\s*column\s*!important/
		);
	});

	it('.doc-meta-row uses display: flex !important', () => {
		expect(printBlock).toMatch(
			/\.doc-meta-row\s*\{[^}]*display:\s*flex\s*!important/
		);
	});

	it('.doc-dates-row has reduced margin-top', () => {
		expect(printBlock).toMatch(
			/\.doc-dates-row\s*\{[^}]*margin-top:\s*2px\s*!important/
		);
	});
});
