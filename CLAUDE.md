...
- **Folder tree per source** — The Sidebar still renders the recursive `TreeNode.svelte` concertina built from `buildFolderTree` (pure helper in `src/lib/tree.ts`). Both source routes — root (`/source/[name]`) and folder (`/source/[name]/[...path]`) — render **one flat table per directory** (no nesting). On the root route each directory is a sibling group with `Root Documents` first; on the folder route the current folder's docs render in a `Files` group and any descendant directories become sibling groups. Columns on both routes: **Title / Filename (basename) / Modified / Created / Lines**, with a Recent / A–Z sort toggle. The folder-route masthead carries a GOV.UK caption-xl above the H1 chaining the source + parent folder labels (e.g. "SRE Agent › Documentation"), and the H1 itself is the formatted current folder. Source-tree data comes from `GET /api/sources/tree` (bulk) and `GET /api/sources/:name/tree` (single); `line_count` is derived server-side from stored content. Breadcrumb segments remain clickable links resolving to the `[...path]` route.
